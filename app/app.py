...
if df_filtered.empty:
    ...
    def create_etc_cost_column(df):
        df['기타비용'] = (df['양식'] + df['한식'] + df['용도품_침구'] + df['용도품_물품'] +
                        df['예선료'] + df['통선비'])
        return df
    ...
    df_filtered = create_etc_cost_column(df_filtered)

    statuses = ['정상', '지연', '전체']
    status_labels = {'전체': 'Total', '정상': 'Normal', '지연': 'Delayed'}

    cost_types = ['유류비(\)', '인건비', '기타비용', '총 경비']
    cost_type_labels = {'유류비(\)': 'Fuel Cost', '인건비': 'Labor Cost', '기타비용': 'Other Cost', '총 경비': 'Total Cost'}
    ...
    with st.expander("비용 분석 결과 보기"):
        for status in statuses:
            ...
            st.write(f"- Labor Cost: {data[status][1]:.2f}")
            st.write(f"- Other Cost: {data[status][2]:.2f}")
            st.write(f"- Total Cost: {data[status][3]:.2f}")
    ...
    with st.expander("Radar Chart 설명 보기"):
        st.write("**Fuel Cost**: 시운전 선박이 사용한 기름값을 나타냅니다.")
        st.write("**STN_C**: Sea Trials Navigator Cost의 약자로 항해사비가 됩니다. 항해사비는 선장 비용, 타수 비용, 도선비, 임시항해 검사비, 자차 수정 비용 이 포함된 금액입니다.")
    ...
    def plot_radar_chart(ratios):
        categories = ['Fuel Cost', 'STN_C', 'SMMT_C', 'Total Cost', 'Other Cost']
    ...
    if st.button("이 페이지를 PDF로 저장"):
        try:
            ...
            url = st.get_option('browser.gathered_url')
